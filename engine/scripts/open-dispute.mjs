...
async function main() {
  ...
  await send(buyer, d.escrow, ESCROW, "fileDispute", [paymentId, claimType, []], "file dispute");
  ...
  console.log(`  curl -X POST localhost:8080/api/demo/attest  -H 'content-type: application/json' -d '{"paymentId":${paymentId},"value":2}'`);
  console.log(`  curl -X POST localhost:8080/api/demo/resolve -H 'content-type: application/json' -d '{"paymentId":${paymentId}}'`);
