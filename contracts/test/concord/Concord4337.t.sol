...
import {ConcordTestBase} from "./ConcordTestBase.sol";
...
contract Concord4337Test is ConcordTestBase {
    uint48 constant UNTIL = type(uint48).max;

    function consumer() internal returns (Concord) {
    ...
    function deviceAlice(Concord account, PackedUserOperation memory op) internal view returns (PackedUserOperation memory) {
    ...
    function test_thresholdTransferAsUserOperation() public {
        ...
        emit IConcord.Executed(hash, op.nonce, PATH_THRESHOLD);
        submit(op);
    ...
    function test_onlyExecuteUserOpCallDataIsServed() public {
        Concord account = plainAccount();
        PackedUserOperation memory op = userOp(account, one(transferCall(dave, 1e6)), 0);
        op.callData = abi.encodeCall(Concord.approve, (keccak256("h")));
        bytes32 hash = opHash(account, op, 0, UNTIL);
        op = withSignature(op, 0, UNTIL, aliceBob(hash));
        PackedUserOperation[] memory ops = new PackedUserOperation[](1);
        ops[0] = op;
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IEntryPointErrors.FailedOpWithRevert.selector, 0, "AA23 reverted", abi.encodeWithSelector(IConcord.BadCallData.selector)
            )
        );
        IEntryPoint(ENTRY_POINT).handleOps(ops, payable(bundler));
    }
    ...
    function test_singleSignerSpendAsUserOperation() public {
        ...
        setup[0] = selfCall(account, abi.encodeCall(Concord.setSpendingLimit, (0, SpendingLimitInput(address(usdc), 0, 100e6, 1 days, true))));
        setup[1] = selfCall(account, abi.encodeCall(Concord.allowLimitSigner, (1, idOf(deviceX, deviceY))));
        runAliceBob(account, setup);

        PackedUserOperation memory op = userOp(account, one(selfCall(account, abi.encodeCall(Concord.spend, (1, dave, 30e6)))), 0);
        bytes32 hash = opHash(account, op, 0, UNTIL);
        op = withSignature(op, 0, UNTIL, pack1(idOf(deviceX, deviceY), signP256(devicePk, hash)));
        vm.expectEmit(true, false, false, true);
        emit IConcord.Executed(hash, op.nonce, PATH_SINGLE);
        submit(op);
        assertEq(usdc.balanceOf(dave), 30e6);

        // A signer not on the limit is refused at validation.
        op = userOp(account, one(selfCall(account, abi.encodeCall(Concord.spend, (1, dave, 1e6)))), 0);
    ...
    function test_deviceVetoesARecoveryAsUserOperation() public {
        ...
        Call[] memory calls = one(selfCall(account, abi.encodeCall(Concord.replaceSigner, (idOf(deviceX, deviceY), p256(newX, newY, PERM_APPROVE | PERM_VETO)))));
        ...
        PackedUserOperation memory op = userOp(account, one(selfCall(account, abi.encodeCall(Concord.veto, (hash)))), 0);
    ...
    function test_configThroughAUserOperationIsScheduledAndVetoable() public {
        Concord account = consumer();
        PackedUserOperation memory op = deviceAlice(account, userOp(account, one(selfCall(account, abi.encodeCall(Concord.setThreshold, (1)))), 0));
        bytes32 hash = opHash(account, op, 0, UNTIL);
        submit(op);
        ...
        account.executeScheduled(hash, one(selfCall(account, abi.encodeCall(Concord.setThreshold, (1)))));
    ...
    function test_aCancelledOperationDoesNotValidate() public {
        ...
        bytes32 hash = opHash(account, op, 0, UNTIL);
        op = withSignature(op, 0, UNTIL, aliceBob(hash));
        runAliceBob(account, selfCall(account, abi.encodeCall(Concord.cancel, (hash))));
    ...
    function test_executeUserOpNeedsTheEntryPointAndAValidation() public {
        Concord account = plainAccount();
        PackedUserOperation memory op = userOp(account, one(transferCall(dave, 1e6)), 0);
        vm.expectRevert(IConcord.NotEntryPoint.selector);
        account.executeUserOp(op, keccak256("x"));
        vm.prank(ENTRY_POINT);
        vm.expectRevert(IConcord.NotValidated.selector);
        account.executeUserOp(op, keccak256("x"));
    }
    ...
    function test_counterfactualAccountDeploysFromInitCode() public {
        ...
        Concord account = Concord(payable(predicted));

        PackedUserOperation memory op;
        op.sender = predicted;
        op.nonce = 0;
        op.initCode = abi.encodePacked(address(factory), abi.encodeCall(ConcordFactory.createAccount, (init, "cf")));
        ...
        bytes32 hash = hasher.userOp(
            keccak256(abi.encode(keccak256("EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)"), keccak256("Concord"), keccak256("1"), block.chainid, predicted)),
    ...
    function test_twoOperationsInOneBundleKeepTheirOwnPaths() public {
        ...
        PackedUserOperation memory change = deviceAlice(account, userOp(account, one(selfCall(account, abi.encodeCall(Concord.setThreshold, (1)))), 1));
