    ...
    private var methods: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            LazyVGrid(
                ...
                // An address, not a wallet to connect. The USDC sent to it is burned on
                // Base and minted on Arc by Circle, so what lands is real Arc USDC and
                // no gas on Arc is needed to receive it. Asking somebody to connect a
                // wallet would turn a money app back into a crypto app; this is the way
                // every other money app does it, and the address can only pay them.
                NavigationLink {
                    BridgeDepositView(environment: environment)
                } label: {
                    DepositCard(
                        icon: "arrow.left.arrow.right",
                        title: "Another chain",
                        detail: "USDC from Base, sent to your address",
                        marks: [.base, .usdc],
                        availability: .live
                    )
                }
