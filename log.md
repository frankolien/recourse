...
Built: backend ARC_RPC_URL switched to the drpc gateway and verify_buyer's chain read given a three-attempt backoff. Cloudinary mirror for product images (services/cloudinary.rs): signed uploads (SHA-256 signature, reusing the sha2 dependency; Cloudinary detects the algorithm by length) with public_id = the image's keccak hex, a .cdn sidecar recording the delivery URL, and GET /api/orders/image redirecting to the CDN original with an immutable cache header; the volume copy stays the source of truth, a mirror failure only logs, and no CLOUDINARY_URL means the feature is off. Content addressing makes the redirect trustless: the buyer rehashes whatever bytes arrive, and a live round trip proved Cloudinary returns the original byte-for-byte. iOS: checkout publish now blocks while the product photo is still loading. Web: the hero mock cards replaced with three real iPhone screenshots (home, protected payment 13, verify outcome) fanned in dark phone frames with a glow and a floating chip, next/image with CDN optimization (262 KB PNG delivered as 54 KB WebP); the collage CSS (489 lines) deleted; site aliased to recourse-arc.vercel.app with backend CORS extended and preflight verified.

Settled live on Arc (R8, attestor runs logged): payment 13 (Fish, $5.20), disputed NOT_DELIVERED from the owner's iPhone with Face ID-signed photo evidence, attested value 2 by the demo attestor from a local DEMO_MODE backend against real Arc (attestation tx 0xb85cfab7d1cc6ad6ff6de4180096fec302f231cbd6e4ff1ed043efa3e2c35348, digest 0x23110dbbd672fed1eeb01781d58e0edb7846ac50f9d929ba1ec53919671daf89), then resolved (tx 0x6c9eabc8bfd889215771ac6315dfacfb663a2ee934411f8b5d96cac48f485351): rule 0 matched, verdict 10000 bps, buyer 5.772210 to 10.972210 USDC, an exact 5.20 refund. Payment 15 (Watch, $2.00), disputed Wrong item, matched no rule in policy 4 (rules cover only Not delivered and Damaged), resolved to a deterministic 0 bps (tx 0x209890e66483ce0eb2f53f5a6f4a63e05242389d8d631a8c5e6b7b86875e5408): merchant 19.984866 to 21.985149 USDC, the escrowed 2.00 plus 0.000283 of accrued vault yield. One refund and one denial, both previewable before settlement and both reproducible from public state.

Rules earned: R14, interactive request paths get the same RPC retry discipline as background jobs; a retry that exists only in the indexer is a bug waiting for the first real user. R15, any async media selection must gate the action that consumes it; optional attachments that load in the background fail silently otherwise.

---

## 2026-07-25: Session 37, first real device payment, native Google sign-in, and orderRef event indexing

Found: the first end-to-end attempt from a physical iPhone surfaced a chain of real-world failures. Continue with Apple greyed out (stale build with the old localhost default), then payment failing at the first chain read: rpc.testnet.arc.network per-IP rate limits 429 phones on carrier NAT, surfaced as an opaque invalidRPCResponse because a non-2xx HTTP status bypassed the JSON-RPC error path. After the payment succeeded, Home rows showed a spinner forever (AsyncImage with a nil URL never leaves .empty) and a bare merchant address, because the indexer only knows chain state and the escrow's readable state never stores orderRef (only the Paid event does), so nothing could link a payment back to its verified order manifest. Separately: every GitHub push triggered a Railway deploy that failed its healthcheck despite identical code and config (CLI deploys 5 for 5, GitHub 0 for 4), and with the volume attached each failed push-deploy took production down.

Built: native Google sign-in with no SDK (OAuth authorization-code + PKCE through ASWebAuthenticationSession, state echo, backend verifies the ID token audience against GOOGLE_IOS_CLIENT_ID). Switched the app RPC to the drpc gateway via arc-config.json. Cinematic payment-success choreography (haptics, springing shield with one-shot pulse ring, eased amount count-up, cascading rows, subdued aurora; Reduce Motion collapses to a still). The image fix in three layers: artwork falls back instantly on a nil URL; the buyer stores the verified manifest's itemName and imageHash per payment at pay time; and the durable link, migration 0007 plus an indexer sweep that backfills payments.order_ref from Paid event logs (binary-search the block height near the earliest missing payment's paidAt, then chunked getLogs with the existing spacing and retries, self-quieting when nothing is missing), with the API serving orderRef and the app resolving any context-less payment by fetching the manifest and re-verifying its hash before caching.

Verified live: payment 13 was paid from the owner's iPhone ($5.20, item Fish, with a photo), indexed, and after the sweep its orderRef (0xb1d246b8..6cf1) resolved to the manifest whose keccak matches exactly, with the 308 KB product photo served from the volume; the owner confirmed name and image render in the app. iOS suite 52 passed 0 failed throughout; backend 19 passed, clippy and fmt clean. Railway config fixed along the way: volume recourse-volume mounted at /data (found missing by audit; evidence and orders now survive redeploys), GOOGLE_CLIENT_ID re-set (stored value was a truncated masked paste with a literal ellipsis), GOOGLE_IOS_CLIENT_ID set. Open owner action: disable Auto Deploy on the recourse service; until then every push briefly replaces prod with a failing build.

Rules earned: on mobile, assume carrier NAT shares your IP with thousands of users; pick RPC endpoints accordingly and surface HTTP-status failures with their status, never as a generic invalid-response. When state does not store a fact the product needs, check the event log before inventing a side channel; the chain usually already emitted it. AsyncImage with a nil URL stays in .empty forever; branch before constructing it. When two deploy paths run identical code and one always fails, compare the deployment metadata field by field before touching the code.

---

## 2026-07-24: Session 36, order manifests, product images, profile persistence, and a parallel-agent verification pass

Found: the merchant checkout only carried an amount and a hashed order reference, so the buyer reviewed a payment against hardcoded display data (CloudCompute) rather than what they were actually buying. The product model requires per-checkout order details (item, description, optional image) cryptographically bound to the on-chain bytes32 orderRef. Also: profile name edits lived in local AppStorage only, the ContractABITests expectation predated registerPolicy, and the first suite run silently targeted a simulator that no longer exists (iPhone 16 Pro is on the stale 18.4 runtime; iPhone Air is current).

Decided (design): the order manifest is an exact JSON document and orderRef = keccak256(manifest bytes), the same value pay() receives. Hash-of-exact-bytes sidesteps cross-language JSON canonicalization entirely: the backend stores the verbatim bytes content-addressed (like evidence blobs), the buyer refetches by hash, rehashes, and only then parses and cross-checks every economic field (chain, escrow, merchant, policy, amount) against the scanned QR. The backend is trusted for transport, never content. QR v2 is the same envelope with v:2 signaling that orderRef resolves to a manifest; v1 stays decodable and payable.

Built (backend): services/orders.rs (OrderStore: verbatim manifest bytes keyed by keccak, content-addressed product images with content-type, structural validation) and handlers/orders.rs (POST /api/orders validates deployment match and that a referenced image was uploaded; POST /api/orders/image whitelists image types at 5 MB; GETs serve exact bytes), wired in app.rs with ORDERS_DIR (Dockerfile: /data/order-store). Built via a parallel agent: PUT /api/me/profile (bearer-authenticated like /api/me, trim, empty clears, 80-char cap, deny_unknown_fields, returns the /api/me shape; PUT added to CORS methods). No migration needed (accounts name columns existed since 0003).

Built (iOS): OrderManifest (decode-verifying-hash, crossCheck against the request, encodedForPublishing), OrderAPIClient (rehash-verifying image and manifest fetches), PaymentRequest v2 (validate accepts 1 and 2). Merchant checkout gains item name, description, and a PhotosPicker product photo (preview, remove, downscale to 1200px JPEG before upload) and publishes image then manifest then renders the v2 QR. Buyer CheckoutReviewView fetches and verifies the manifest before enabling payment: verified orders show the real item, description, image, and merchant; any mismatch (hash, chain, escrow, merchant, policy, amount, tampered image) blocks payment with a clear reason and retry; the hardcoded CloudCompute display data is gone. Profile editing now persists through the backend (AccountSession.updateProfile with a one-shot refresh retry) and the AppStorage name keys are removed; Home and Settings read the account session.

Verified: Rust 19 tests, clippy and fmt clean. iOS 52 tests, 0 failures (2 intentionally skipped integration tests), including OrderManifestTests (golden, tamper, crosscheck, roundtrip) and PaymentRequest v1/v2/unknown-version. Engine vitest asserts the same golden orderRef via viem, so Swift, Rust, and TypeScript provably agree on 0xa4e9..b7cd for the shared fixture. A parallel agent booted the backend live and smoke-tested the order endpoints 12/12 (hash matches computed independently, byte-identical roundtrips, all negative cases correct, idempotent re-post). Another agent audited Railway production read-only: found the CRITICAL missing /data volume (evidence, and soon orders, are wiped every redeploy), a truncated GOOGLE_CLIENT_ID (contained a literal ellipsis from a masked paste; fixed with the full 72-char id, skip-deploys), GOOGLE_IOS_CLIENT_ID unset, and everything else healthy. Volume attach and the iOS Google client id remain owner actions.

Rules earned: bind off-chain documents to the chain by hashing exact bytes, never by canonicalizing JSON across languages; store verbatim, verify by rehash on every read. When a test targets a named simulator, pin one that exists on the current runtime (a wrong destination fails the whole run before a single test). An audit that only reports variable presence and shape can still find real outages: a masked value pasted into a secret store looks set but can never validate.

---

## 2026-07-23: Session 35, deployment readiness, CORS, mobile Google, email/password, and passkeys

Found: the backend was feature-complete for the four mobile asks (buyer filtering, pagination, EIP-712 wallet auth, authenticated evidence upload/manifest are all in) but had no deployment path: no Dockerfile, no host config, CORS wide open (Cors::permissive), Google verification bound to a single web audience (so iOS Google Sign-In could not use the same endpoint), and no non-social login. Owner wired their Google Web client id (181083896548-...apps.googleusercontent.com) into backend/.env GOOGLE_CLIENT_ID and web/.env.local NEXT_PUBLIC_GOOGLE_CLIENT_ID.

Built (deploy): repo-root Dockerfile (multi-stage; stub-main dep layer so alloy caches across edits; runtime bundles deployments/arc-testnet.json since addresses are read at runtime, R3), .dockerignore, and railway.json (Dockerfile builder, /health healthcheck). Deploying on Railway: a Postgres service (DATABASE_URL reference) plus a volume mounted at /data for the evidence blob store; Railway injects PORT and the app binds to it. sqlx::migrate! embeds migrations and runs them on boot, so a fresh DB self-provisions. README gained a Railway deploy runbook. (Started on Fly, switched to Railway mid-session; the Dockerfile is provider-agnostic, so only the platform config and runbook changed.)

Built (CORS): CORS_ALLOWED_ORIGINS (comma-separated) in AppConfig; app.rs build_cors restricts to those origins (GET/POST/OPTIONS + content-type/authorization/x-recourse-auth) when set, and falls back to Cors::permissive only when empty (dev/demo). Locked down in production via a Fly secret.

Built (mobile Google): GoogleAuthService now holds audiences: Vec<String> instead of one client_id and accepts a token whose aud is any configured client id. from_config builds the set from GOOGLE_CLIENT_ID (web) + GOOGLE_IOS_CLIENT_ID (iOS), so POST /api/auth/google serves web and iOS unchanged.

Built (email/password): migration 0005 adds accounts.password_hash (nullable; only provider='email' rows use it). account_sessions::register_email/login_email hash with Argon2id (fresh random salt, PHC string, never plaintext); register is 409 on a duplicate email (unique-violation 23505 mapped), 400 on a malformed email or a password under 8 chars; login is a uniform 401 so it never reveals which emails exist. New handlers POST /api/auth/email/register and /login, wired in app.rs. AccountAuthError gained BadRequest(400) and Conflict(409).

Built (passkeys, iOS-first): server-side WebAuthn via webauthn-rs 0.5 (feature danger-allow-state-serialisation so the in-flight ceremony state can be parked in Postgres). Validated the exact 0.5.5 API with a research subagent before coding. Migration 0006 adds passkey_credentials (account_id, unique credential_id BYTEA, the serialized Passkey JSONB) and webauthn_ceremonies (single-use, 5-min, consumed by DELETE ... RETURNING). services/passkey wraps a configured Webauthn (WEBAUTHN_RP_ID/RP_ORIGIN; disabled -> 503). Four email-first endpoints: POST /api/auth/passkey/{register,login}/{start,finish}. Register creates the provider='passkey' account only on finish (after attestation verifies) and issues the same opaque session; login verifies the assertion and bumps the credential signature counter. account_sessions gained the ceremony store/take, passkey account loaders, create_passkey_account, and issue_passkey_login. iOS needs a real https domain: rp_id is the app's apple-app-site-association webcredentials domain, rp_origin is https://<rp_id>, plus the Associated Domains entitlement, so passkeys are tested against the deployment, not localhost.

Verified: cargo check clean (exit 0); cargo test 14 pass, including a new Argon2 hash round-trip (asserts the stored value is a $argon2 PHC string, not the plaintext, and that a wrong password fails) and email normalization/validation; clippy and fmt clean (build_app's argument count is annotated). The passkey ceremonies compile against the real webauthn-rs API but have no unit tests (they need a live authenticator); they are covered by types + the compile, to be exercised end to end from the iOS client against the deployed https domain. A note for the environment: the local disk hit 100% mid-session (the test profile plus the webauthn+alloy tree); freed by rm -rf backend/target, and builds now use CARGO_PROFILE_DEV_DEBUG=0 to keep target small. Not yet run live: the Fly deploy itself, the mobile Google flow (needs the iOS client id + device), email register/login against Postgres, and the full passkey ceremony (needs the deployed domain + an iOS device).

Rules earned: ship deployment as code (a provider-agnostic Dockerfile + the platform's config file + a runbook), and read runtime inputs from files bundled into the image rather than the build host. CORS should fail safe by config: permissive only as an explicit dev fallback, locked to the web origin the moment an allow-list is set. When a provider issues per-platform audiences (Google web vs iOS), verify against the whole set, not one id. For a password path, store only an Argon2id hash, return a uniform login error so the endpoint is not an account-enumeration oracle, and map the DB unique violation to a real 409.
